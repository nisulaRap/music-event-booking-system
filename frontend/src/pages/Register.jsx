...
import { Music2, Mail, Lock, User, Phone, Loader2, UserPlus } from 'lucide-react'
...
export default function Register() {
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      if (data.token) { login(data); toast.success('Account created!'); navigate('/') }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center px-6 pt-20 pb-10">
      <div style={{ width: '100%', maxWidth: '420px' }}>
        <div className="animate-fade-up anim-delay-1" style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', fontWeight: 700, color: 'white' }}>Create account</h1>
          <p style={{ color: '#9ca3af', marginTop: '0.5rem', fontSize: '0.9rem' }}>Join Encore and book your first event</p>
        </div>
        ...
        <div className="card animate-fade-up anim-delay-2" style={{ padding: '2rem' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }} noValidate>
            <Field label="Full Name" icon={<User size={14} />} error={errors.name}>
              <input type="text" value={form.name} onChange={set('name')} placeholder="John Perera" className={`input-field ${errors.name ? 'border-red-500' : ''}`} />
            </Field>
            ...
            <Field label="Password" icon={<Lock size={14} />} error={errors.password}>
              <input type="password" value={form.password} onChange={set('password')} placeholder="Min. 6 characters" className={`input-field ${errors.password ? 'border-red-500' : ''}`} />
            </Field>
            <Field label="Phone (optional)" icon={<Phone size={14} />}>
              <input type="tel" value={form.phone} onChange={set('phone')} placeholder="+94 77 123 4567" className="input-field" />
            </Field>
          ...
          <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.875rem', color: '#6b7280' }}>
            ...
            <Link to="/login" style={{ color: 'var(--color-gold-400)', fontWeight: 600, textDecoration: 'none' }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
